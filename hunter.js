/**
 * WALLET HUNTER v14.1 — three ways to find a wallet, plus early-buyer tracing
 * ----------------------------------------------------------------
 ...
 * wallet found was one that happened to trade a coin we picked, and we could
 * only read coins whose whole history fits in the Helius page budget.
 *
 ...
 * and whales. No ranking-based source can reach a trader of 0.3 SOL size.
 *
 * So v13 carries THREE sources, none of which rank by profit:
 *   1. CO-OCCURRENCE — wallets that keep turning up in the same coins as
 *      wallets already trusted.
 *   2. /trace — wallets common to a set of coins you name.
 *   3. Birdeye smart_trader — fallback.
 *
 ...
 *
 * v14.1: PDA derivation is done natively (SHA-256 + ed25519 curve check,
 * verified against real keypairs) so there is no @solana/web3.js dependency.
 *
 * Also fixed in v14: MIN_BUY_SOL was referenced but never defined, so dust
 * buys were never filtered anywhere.
 *
...
/* ── native base58 + PDA derivation (no web3.js) ───────── */
...
const edMod = (a) => ((a % ED_P) + ED_P) % ED_P;
function edPow(b, e) {
  let r = 1n; b = edMod(b);
...
/* a PDA is a 32-byte hash that does NOT decompress to a point on ed25519 */
function isOnCurve(bytes32) {
  const y = edMod(BigInt("0x" + Buffer.from(bytes32).reverse().toString("hex")) & ((1n << 255n) - 1n));
  if (y >= ED_P) return false;
  const y2 = y * y % ED_P;
  const u = edMod(y2 - 1n);
  const v = edMod(ED_D * y2 + 1n);
  const x2 = u * edInv(v) % ED_P;
  if (x2 === 0n) return true;
  let x = edPow(x2, (ED_P + 3n) / 8n);
  if (x * x % ED_P !== x2) x = x * edPow(2n, (ED_P - 1n) / 4n) % ED_P;
  return x * x % ED_P === x2;
}
...
/* pump.fun bonding-curve account for a mint */
function curvePda(mint) {
  return findProgramAddress([Buffer.from("bonding-curve"), b58decode(mint)], PUMP_PROGRAM);
}
...
async function traceCoins(mints, control, opts) {
  const mode = opts.mode === "mint" ? "mint" : "curve";
  const perCoin = [];
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  <div class="sub">v14.1 · co-occurrence + smart_trader + /trace (curve mode) → Helius vetting · cycle ${CFG.CYCLE_MINUTES}m · top ${CFG.SHORTLIST_SIZE} · ${persistOk ? "database saved ✓" : "IN MEMORY ONLY"} · <a href="/db.json" style="color:var(--green)">db.json</a> · <a href="/probe" style="color:var(--green)">probe</a> · <a href="/trace" style="color:var(--green)">trace</a></div>
...
http.createServer(async (req, res) => {
  ...
  /* quick self-check: /pda?mint=<mint> shows the derived bonding-curve address */
  if (p === "/pda") {
    const mint = u.searchParams.get("mint") || "";
    try {
      res.writeHead(200, { "Content-Type": "application/json" });
      return res.end(JSON.stringify({ mint, bondingCurve: curvePda(mint) }, null, 2));
    } catch (e) {
      res.writeHead(400, { "Content-Type": "application/json" });
      return res.end(JSON.stringify({ mint, error: e.message }, null, 2));
    }
  }
  ...
  console.log(`Wallet hunter v14.1 on ${CFG.PORT} — persistence ${persistOk ? "ON" : "OFF"}`);
